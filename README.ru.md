...
`stride-align` — это библиотека, которая показывает, насколько «похожи» две строки. Она делает это, реализуя алгоритмы Smith-Waterman и Needleman-Wunsch. Вместо лекции будем учиться на примерах. Сразу посмотрим, как это работает.
...
**LoongArch / Loongson.** История оптимизации Loongson особенно показательна: для вычислений только оценки на проверенном тесте бэкенд LASX в 16 раз быстрее универсального бэкенда и в **22,3 раза** быстрее Parasail.
