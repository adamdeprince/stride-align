#!/bin/bash
# Build LoongArch64 old-world (GCC 15.2.0) + new-world (GCC 16.1.0 wrappers)
# wheels for cp312/313/314 from the 0.5.1 sdist. See docs/loongson-build.md.
#
...
set -uo pipefail
SDIST=/tmp/stride_align-0.5.1.tar.gz
BW=/tmp/sa-bw
  ...
  for pair in 3.12:3.12.13 3.13:3.13.13 3.14:3.14.4; do
    ...
    CC="$2" CXX="$3" PATH="$CMAKE_BIN:$4:$PATH" \
      "$venv/bin/pip" wheel "$SDIST" --no-build-isolation --no-deps \
        -w "$HOME/wheelhouse/$1" 2>&1 | grep -iE "created|error|fail" | tail -2
  done
}

GCC15=/opt/loongson-gcc-15.2.0; GCC16=/opt/loongson-gcc-16.1.0
build oldworld "$GCC15/bin/gcc"      "$GCC15/bin/g++"      "$GCC15/bin"
build newworld "$GCC16/wrappers/gcc" "$GCC16/wrappers/g++" "$GCC16/wrappers:$GCC16/bin"

# Insert the PEP 427 build tag into each filename (filename-only; safe).
for world in oldworld newworld; do
  for w in "$HOME"/wheelhouse/$world/stride_align-0.5.1-cp*.whl; do
    [ -e "$w" ] || continue
    b=$(basename "$w")
    mv "$w" "$HOME/wheelhouse/$world/${b/stride_align-0.5.1-/stride_align-0.5.1-1.$world-}"
  done
done
