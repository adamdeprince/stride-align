...
stride-align 通过 Python、R 与 DuckDB 公开同一套原生算法。本页为每种算法
都给出三个接口的调用方式。任意示例上的语言选择器都会与本页其他选择器及
首页同步，因此无论读到哪里都可以切换接口。

以下示例假定 Python 中已经执行 `import stride_align as sa`，R 中已经执行
`library(stridealign)`，或者 DuckDB 已经加载 stride-align 扩展。请参阅
[中文首页的安装演练](../../index.zh-CN.html#install)，或按 DuckDB 版本、平台
与处理器选择并[下载 DuckDB 软件包](https://distribution.goblinreactor.com/stride-align/duckdb/index.zh-CN.html)。

本编程接口按算法而不是语言适配器组织。Python 与 R 使用普通函数调用；DuckDB
使用 `stride_*` SQL 函数处理字面量、列或列表值。本原生跨语言参考不包含
兼容层。
...
| `cdist` | `Q` 个查询与 `T` 个目标 | 稠密的 `Q × T` 分数矩阵 |
