...
stride-align 通过 Python、R、Go、DuckDB、PostgreSQL 与 Memgraph 公开其原生算法。
本页为每种算法都给出六个接口的调用方式。任意示例上的语言选择器都会与
本页其他选择器及首页同步，因此无论读到哪里都可以切换接口。

以下示例假定 Python 中已经执行 `import stride_align as sa`，R 中已经执行
`library(stridealign)`；Go 面板给出完整程序，数据库面板则使用各自代码中
显示的 Python 客户端。
...
Go 的安装与 SIMD 构建配置见 [Go 软件包指南（英文）](../../bindings/go/)，
没有 C++23 编译器时可使用[预编译原生后端](https://distribution.goblinreactor.com/stride-align/go/index.zh-CN.html)。

本编程接口按算法而不是语言适配器组织。Python、R 与 Go 使用普通函数调用；
...
| Go | 结果使用切片、嵌套切片和带类型的匹配结构体，索引从零开始。面向切片的操作会将字符串扁平化，每次调用只进入 C++ 一次；`CDistTopK` 在全局筛选，而 `CDistTopKPerQuery` 为每个查询返回一组结果。 |
