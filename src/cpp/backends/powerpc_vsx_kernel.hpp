...
namespace stride_align::powerpc_vsx_kernel {
  ...
  for (std::size_t diagonal = 2; diagonal <= query.size() + target.size(); ++diagonal) {
    ...
    for (std::size_t offset = 0; offset < current.scores.size(); offset += lane_count) {
      ...
      for (std::size_t lane = 0; lane < count; ++lane) {
        ...
        block.query_tokens[lane] = query[row - 1];
        block.target_tokens[lane] = target[column - 1];
        block.diagonal_scores[lane] = lookup_score<LocalAlignment>(
            previous_previous,
            row - 1,
            column - 1,
            gap_score);
        block.up_scores[lane] = lookup_score<LocalAlignment>(previous, row - 1, column, gap_score);
        block.left_scores[lane] = lookup_score<LocalAlignment>(previous, row, column - 1, gap_score);
      }

      const auto substitution_vector =
          Ops::substitution(block.query_tokens, block.target_tokens, match_score, mismatch_score);
      const auto diagonal_vector = Ops::add(Ops::load_cells(block.diagonal_scores), substitution_vector);
      const auto up_vector = Ops::add(Ops::load_cells(block.up_scores), gap_vector);
      const auto left_vector = Ops::add(Ops::load_cells(block.left_scores), gap_vector);

      Ops::store_cells(block.diagonal_scores, diagonal_vector);
      Ops::store_cells(block.up_scores, up_vector);
      Ops::store_cells(block.left_scores, left_vector);
      ...
      if constexpr (Ops::has_vector_max) {
        ...
        Ops::store_cells(block.cell_scores, cell_vector);
      }
      ...
      for (std::size_t lane = 0; lane < count; ++lane) {
        ...
        Cell selected_score = block.diagonal_scores[lane];
        TraceDirection direction = TraceDirection::diagonal;

        if (block.up_scores[lane] > selected_score) {
          selected_score = block.up_scores[lane];
          direction = TraceDirection::up;
        }

        if (block.left_scores[lane] > selected_score) {
          selected_score = block.left_scores[lane];
