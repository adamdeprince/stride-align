...
Score score(
  ...
  const auto profile = build_profile<OpsTemplate, Cell>(query, match, mismatch, segment_count);
  ...
  for (const std::uint8_t target_token : target) {
    ...
    for (std::size_t segment = 0; segment < segment_count; ++segment) {
      ...
      const auto v_profile = Ops::load_cells(profile_segment);
      auto v_e = Ops::load_cells(e_segment);
      ...
      Ops::store_cells(e_segment, v_e);
      v_f = Ops::max(Ops::add(v_f, gap_vector), v_h_gap);
      v_h = Ops::load_cells(h_load_segment);
    }
